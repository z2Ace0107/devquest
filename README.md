...
## 技术复用：DevQuest → MCP 智能工单 Agent

DevQuest 是我 MCP + ChromaDB + LangChain 技术栈的**实验田**。在这上面验证了工具设计、检索方案和协议集成后，这些经验直接复用到另一个主项目——[MCP 智能工单 Agent 系统](../MCP智能工单Agent系统/mcp-agent-tickets/)：

| 复用内容 | DevQuest（本项目） | MCP 工单 Agent（主项目） |
|----------|-------------------|------------------------|
| MCP Server 标准化 | 11 个工具，JSON-RPC 2.0 stdio | 12 个工具，同一套模式 |
| ChromaDB + 阿里百炼 Embedding | 向量语义检索通道 | RAG `search_solutions` |
| LangChain LLM 集成 | extractor → classifier → scorer 管道 | Supervisor → 4 Agent 编排 |
| 工具描述设计 | 总结出 description 怎么写 LLM 才选得准 | 直接复用方法论 |
| RRF 双通道融合 | 语义 + FTS5 关键词联合检索 | 启发了工单项目的搜索排序设计 |
